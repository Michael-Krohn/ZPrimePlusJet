...
#    tree.SetBranchStatus("maxPUptHat",1)
    ...
    tree.SetBranchStatus("AK8PuppiJets_pt[AK8PuppiJets_num]",1)
    tree.SetBranchStatus("AK8PuppiJets_eta[AK8PuppiJets_num]",1)
    tree.SetBranchStatus("AK8SoftDropMass[AK8PuppiJets_num]",1)
    tree.SetBranchStatus("AK8PFJetsCHS_DoubleSV[AK8PuppiJets_num]",1)
    for ev in tree:
#        if not ev.run==297292: continue
	if not (ev.AK8PuppiJets_pt[0] < 1000 and ev.AK8PuppiJets_pt[0]>400 and abs(ev.AK8PuppiJets_eta[0])<2.5 and ev.AK8SoftDropMass[0]>40 and ev.AK8PFJetsCHS_DoubleSV[0] > 0.): continue
        den+=1
...
for signal in ["../GluGluHToBB.root"]:
