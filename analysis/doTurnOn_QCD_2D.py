...
def make2DHistos():
	...
	print "var2=",var2
        ...
        print "binning=",str(binning)
        print "weight=",weight
        tree.Draw(var+":"+var2+">>num"+str(binning),str("("+preselection+"&&"+trigger+")*"+weight+"/"+NEvents),"",maxev)
        print "Draw:\t",var+":"+var2+">>num"+str(binning),str("("+preselection+"&&"+trigger+")*"+weight+"/"+NEvents)
        num = ROOT.gDirectory.Get("num")
        num = copy.copy(num)
        tree.Draw(var+":"+var2+">>den"+str(binning),str("("+preselection+")*"+weight+"/"+NEvents),"",maxev)
        print "Draw:\t",var+":"+var2+">>den"+str(binning),str("("+preselection+")*"+weight+"/"+NEvents)
        den = ROOT.gDirectory.Get("den")
        den = copy.copy(den)
        print "num,den = ",num.Integral(),den.Integral()
        tempNum.Add(num)
...
fileNames    = [("../QCD80to120.root","2762530."),
...
#trigger         = "(l1Jets_pt[0]>170 && AK8CaloJetsIDPassed_pt[0]>270 && abs(AK8CaloJetsIDPassed_eta[0])<5.0 && HighestAK8PFJetForBTag_csv>0.24 && AK8PFJetsMatchedToCaloJets270_pt[0]>320 && abs(AK8PFJetsMatchedToCaloJets270_eta[0])<5.0)"
trigger         = "(l1Jets_pt[0]>170 && AK8CaloJetsIDPassed_pt[0]>300 && abs(AK8CaloJetsIDPassed_eta[0])<5.0 && HighestAK8PFJetForBTag_csv>0.4 && AK8PFJetsMatchedToCaloJets300_pt[0]>310 && abs(AK8PFJetsMatchedToCaloJets300_eta[0])<5.0 && AK8TrimModJets_mass[0] > 30)"
#trigger         = "HLT_AK8PFJet550_PFAK8BTagCSV_v2==1"
binning         = (NbinsY,yMin,yMax,Nbins,functionMin,functionMax)
ped             = "PFAK8BTagCSV_Trig"
